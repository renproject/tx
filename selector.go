...
const (
	...
	// ChangeSignatoriesFn is an intrinsic function that is generated by block
	// proposers upon detecting a new signatory.
	ChangeSignatoriesFn = "changeSignatories"
	...
	// ClaimFeesFromEventFn is an extrinsic function that is used by Darknode
	// operators to request unclaimed fees earned by their Darknode using a host
	// chain event.
	ClaimFeesFromEventFn = "claimFeesFromEvent"
)
...
// origin chain. This is true for burn-and-release and event-based claim fees
// transactions.
func (selector Selector) IsRelease() bool {
	if selector.IsClaimFees() {
		return true
	}
	asset := selector.Asset()
...
// true for lock-and-mint, burn-and-mint, and claim fees transactions.
func (selector Selector) IsMint() bool {
	if selector.IsClaimFeesFromEvent() {
		return true
	}
	asset := selector.Asset()
	destination := selector.Destination()
...
func (selector Selector) IsCrossChain() bool {
	return selector.IsLock() && selector.IsMint() || selector.IsBurn() && selector.IsRelease()
}
...
// IsClaimFeesFromEvent returns true if the selector is for an event-based claim
// fees tx.
func (selector Selector) IsClaimFeesFromEvent() bool {
	return selector.Fn() == ClaimFeesFromEventFn
}
